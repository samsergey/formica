...
(test-case
 ...
 ; monadic sequencing
 (check-equal? (seq/m (list (return 'a) (return 'b) (return 'c))) (return '(a b c)))
 (check-equal? (seq/m (list (return 'a 'b) (return 'c) (return 'd 'e))) (return '(a c d) '(a c e) '(b c d) '(b c e)))
 (check-equal? (seq/m (list (return 'a) mzero (return 'c))) mzero)
 (check-equal? (seq/m (list (return 'a 'b) mzero (return 'c))) mzero)
 ; monadic sum
...
(test-case
 ...
 (check-equal? (return 'x 'y 'x) (set 'x 'y))
 (check-equal? mzero (set))
 (check-equal? (mplus (set 1 2) (set 2 3)) (set 1 2 3))
 ; the monad laws
 (check-equal? (bind (return 'x) >>= (lift f)) (return (f 'x)))
 (check-equal? (bind (return 'x) >>= F) (return (f 1 'x) (f 2 'x)))
 (check-equal? (bind (return 'x 'y 'z) >>= (lift f)) (return (f 'x) (f 'y) (f 'z)))
...
(test-case
 ...
 ; monadic sequencing
 (check-stream-equal? (seq/m (list (return 'a) (return 'b) (return 'c))) (return '(a b c)))
 (check-stream-equal? (seq/m (list (return 'a 'b) (return 'c) (return 'd 'e))) (return '(a c d) '(a c e) '(b c d) '(b c e)))
 (check-stream-equal? (seq/m (list (return 'a) mzero (return 'c))) mzero)
 (check-stream-equal? (seq/m (list (return 'a 'b) mzero (return 'c))) mzero)
 ; monadic sum
 ...
 (check-equal? (stream-first (seq/m (list (return 1) (stream 0 (/ 0))))) '(1 0))
 (check-exn exn:fail? (λ () (stream-ref (seq/m (list (return 1) (stream 0 (/ 0)))) 1)))
 ; map/m, filter/m and fold/m are not lazy because they operate on eager lists.
 (check-exn exn:fail? (λ () (stream-first (map/m (λ (x) (return x (/ x))) '(1 0 3)))))
